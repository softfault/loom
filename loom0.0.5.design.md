...
这个设计非常大胆且符合 Loom “祛除噪音”的审美。

去掉 `!` 让宏调用看起来像普通函数，这在视觉上极其统一。虽然这要求开发者清楚区分“哪个是宏，哪个是函数”（因为宏可能会导致代码膨胀或改变控制流），但对于 Loom 这种强调 Explicit 的语言，我们可以认为用户在使用导入（`use`）时就已经知道了自己在导入什么。

加上“乱序定义”和“模块化”，这实际上解决了 C 语言预处理器（全局污染、顺序依赖）和 Rust 早期宏（导出困难、必须写在上面）的痛点。
