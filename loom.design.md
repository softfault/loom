...
* **类型语法：** `[]T` (不可变视图), `[]mut T` (可变视图)
* **获取长度：** 使用 `#` 操作符（运行时读取胖指针中的 len 字段）。

### 3.1 创建切片

Loom 提供两种创建切片的方式：从数组隐式转换（安全）和从指针显式构造（不安全）。

#### A. 从数组隐式转换 (Implicit Coercion)

传递数组给接受切片的函数时，编译器会自动构造切片。

```loom
let arr = [1, 2, 3, 4, 5]; // 类型: [5]i32

// 函数签名: fn print_all(items: []i32)
print_all(arr); // 自动转换为 []i32 (ptr=&arr[0], len=5)

```

#### B. 指针构造 (Pointer Slicing)

使用范围语法从原生指针构造切片。这是底层内存操作的核心语法。

* **不可变切片：** `ptr[start..end]` -> `[]T`
* **可变切片：** `ptr[mut start..end]` -> `[]mut T`

> **注意：** 源指针必须具备相应的权限。只有 `*mut T` 才能构造 `[]mut T`。

```loom
// 1. 准备内存 (堆上分配)
let raw: *mut i32 = malloc(1024) as *mut i32;

// 2. 创建只读切片 ([]i32)
// 语义：虽然 raw 是可变的，但我现在只想要一个只读的视图
// 此时 view_ro[0] = 1 是非法的
let view_ro = raw[0..10]; 

// 3. 创建可变切片 ([]mut i32)
// 语法：在括号内显式标记 mut
// 语义：创建一个可读写的视图
let view_rw = raw[mut 10..20];

// 修改数据
view_rw[0] = 99; // 合法：修改的是 raw[10]
...
Loom 中的引用不仅仅是一个指针，它是对现有数据的 **别名 (Alias)**。
引用的设计哲学是 **“接口层面的透明视图”**。它旨在提供一种像操作本地变量一样自然、安全（非空）的访问方式，通常用于函数的参数传递和返回。

与原生指针 `*T` 不同，引用**隐藏**了内存地址的概念，专注于数据的操作。

### 2.1 自动解引用 (Auto-Dereference)

这是引用最显著的特性。当你持有一个引用时，编译器会自动穿透指针层级，让你直接访问目标数据。

* **透明访问：** 不需要使用 `*` 或 `.*` 解引用。对引用进行字段访问 (`ref.field`) 或方法调用，等同于对原值进行操作。
* **禁止显式解引用：** 由于引用被视为数据的别名，**禁止**对引用使用 `.*` 操作符（该操作符仅用于原生指针）。

```loom
struct Player { score: i32 }

// 函数参数使用引用：看起来就像在操作值一样
fn level_up(p: &mut Player) {
    // ✅ 自动解引用：编译器自动处理为 (*p).score += 100
    p.score += 100; 
}

fn main() {
    let mut hero = Player { score: 0 };
    level_up(&mut hero);
    // hero.score 现在是 100
}
...
### 2.2 绑定不可变性 (Binding Immutability)

引用一旦被初始化，它就与目标数据 **“终身绑定”**。

* **引用本身不可变：** 你不能让引用变量重定向（Rebind）去指向另一个地址。
* **赋值即写入：** 对 `&mut T` 类型的变量进行赋值操作（`ref = val`），意味着**修改引用指向的数据**，而不是修改引用本身。

这与 C++ 的引用行为一致，而与 Rust/Go 的引用/指针行为不同。

```loom
let mut a = 10;
let mut b = 20;

let r: &mut i32 = &mut a; // r 绑定到 a

// 赋值操作
r = 30; // ✅ 合法：这等价于 a = 30
        // r 依然绑定在 a 上

// r = &mut b; // ❌ 编译错误：无法对引用变量重新赋值
               // 你不能改变 'r' 指向哪里

```

### 2.3 引用 vs 指针 (Reference vs Pointer)

Loom 严格区分了“别名”与“地址”的语义，以此平衡易用性与底层控制力。

| 特性         | 引用 (`&T` / `&mut T`) | 原生指针 (`*T` / `*mut T`)  |
| ------------ | ---------------------- | --------------------------- |
| **语义**     | **别名 (Alias)**       | **地址 (Address)**          |
| **重定向**   | **不可** (终身绑定)    | **允许** (可随时指向新地址) |
| **访问方式** | **自动** (`ref.field`) | **显式** (`ptr.*.field`)    |
| **空值状态** | 默认非空 (除非 `?&T`)  | 默认非空 (除非 `?*T`)       |
| **指针算术** | **禁止**               | **允许** (仅限 `*mut`)      |
| **主要场景** | 公共 API、参数传递     | 内部实现、数据结构、迭代器  |

> **最佳实践：**
> * 在 **Public API** 中，尽可能使用 `&T` 或 `&mut T`，让调用者获得最流畅的体验。
> * 在 **Private 实现** 或 **迭代逻辑** 中（如链表遍历），将引用转换为指针 (`as *T`) 使用，以便进行重指向或算术操作。
> 
> 

### 2.4 可空引用 (?&T)

虽然标准引用 `&T` 保证非空，但在某些场景（如查找失败）需要表达“没有引用”。此时使用 `?&T`。

`?&T` 同样支持自动解引用，但必须结合 **空值传播操作符 (`.?`)** 或 **解包操作符 (`?`)** 使用。
...
fn start() {
    // 链式调用：如果 get_config 返回 null，则整个表达式短路
    // 如果非空，自动解引用访问 .port
...
#### 2.5.1 栈逃逸检查 (Stack Escape Analysis)

Loom 保留了对引用的基础逃逸分析。编译器会**禁止返回局部栈变量的引用**，防止悬垂指针。
...
#### 2.5.2 无借用检查 (No Borrow Checker)

Loom **不强制** 独占访问规则（Aliasing XOR Mutation）。
这意味着在同一作用域内，你可以同时拥有指向同一数据的 `&T` 和 `&mut T`。内存安全的责任由程序员承担（通常通过逻辑设计保证，而非编译器强加）。

```loom
let mut data = 100;
let r = &data;     // 读引用
let w = &mut data; // 写引用 (Loom 允许这样做)
